(function () {
  ...
  const actualWeightLabels = {
    fineGoldWeight: "Actual Fine Gold Weight",
    alloyWeight: "Actual Alloy Weight",
  ...
  function calculateMetalReference(order) {
    ...
    const requiredFineGold = metalWeightRequired * purityFraction;

    return {
      metalWeightRequired,
      requiredAlloy: Math.max(metalWeightRequired - requiredFineGold, 0),
      requiredFineGold,
      waxWeight
    };
  }
  ...
  function calculateMixedMetalReference({ order, recycledKt, recycledWeight, reference }) {
    ...
    const targetPureGoldRequired = requiredMetalWeight * targetPurity;
    const pureGoldAlreadyPresent = recycled * recycledPurity;
    const additionalFineGoldRequired = Math.max(targetPureGoldRequired - pureGoldAlreadyPresent, 0);
    const remainingMetalToPrepare = requiredMetalWeight - recycled;
    const additionalAlloyRequired = Math.max(remainingMetalToPrepare - additionalFineGoldRequired, 0);

    return {
      additionalAlloyRequired: roundWeightValue(additionalAlloyRequired),
      additionalFineGoldRequired: roundWeightValue(additionalFineGoldRequired),
      pureGoldAlreadyPresent: roundWeightValue(pureGoldAlreadyPresent),
      remainingMetalToPrepare: roundWeightValue(remainingMetalToPrepare),
      targetPureGoldRequired: roundWeightValue(targetPureGoldRequired)
    };
  }

  function getMetalSourceLabel(value) {
  ...
  function KanbanBoard() {
    ...
    function submitOrder(details = {}) {
      ...
      if (submittedInventoryPosting) {
        ...
        try {
          inventoryPostingResult = Inventory.postOrderToInventory(
            {
              ...selectedOrder,
    ...
    return h(
      ...
      {
        className:
          "kanban-shell min-h-[70vh] rounded-lg border border-production-line bg-white shadow-[0_18px_45px_rgba(31,42,42,0.08)]"
      },
      h(
        "div",
        {
        ...
        h(
          ...
          h(
            "button",
            {
              ...
              disabled: !canViewDamagedTrees,
              onClick: openDamagedTrees,
              title: canViewDamagedTrees ? "Open damaged trees" : "View damaged trees permission required",
              type: "button"
            },
            "Damaged Trees"
          ),
        ...
        : h(
            ...
            hasWaxEntries
              ...
              : h(
                  ...
                  {
                    className:
                      "min-h-[46vh] rounded-lg border border-dashed border-production-line bg-slate-50 p-8 text-center text-sm font-bold text-production-muted"
                  },
                  "No wax entries available for casting."
                )
          )
    );
  }
  ...
  function StageColumn({
    ...
    return h(
      ...
      {
        className: cx(
          "kanban-column rounded-lg border border-production-line bg-slate-50/80 p-3",
          isActiveStage && "is-active-stage border-production-teal bg-teal-50/60",
  ...
  function OrderCard({ isActive, onClose, onOpen, order }) {
    ...
    const canOpenOrder = canUseStage(order.stage, "open");
    ...
    return h(
      "article",
      {
        className: cx(
          ...
          order.isRush && "is-rush",
          isActive
            ? order.isRush
              ? "is-active border-amber-500"
              : "is-active border-production-teal"
            : order.isRush
              ? "border-amber-400 hover:border-amber-500"
              : "border-slate-200 hover:border-production-teal"
        )
      },
      h(
        "div",
        ...
        h(
          "button",
          {
            ...
            disabled: !isActive && !canOpenOrder,
            onClick: () => {
              if (!isActive && !canOpenOrder) {
                return;
              }
            ...
            title: isActive ? "Close order" : canOpenOrder ? "Open order" : "Open focused order permission required",
            type: "button",
    ...
    function validateMetalIssue() {
      ...
      requiredFields.forEach((field) => {
        ...
        if (!rawValue) {
          ...
          messages.push(`Enter ${actualWeightLabels[field]}.`);
          return;
        }

        if (!Number.isFinite(parsedValue) || parsedValue < 0) {
          messages.push(`${actualWeightLabels[field]} must be zero or greater.`);
          return;
        }

        if (!isOptionalZeroTopUp && parsedValue <= 0) {
          messages.push(`${actualWeightLabels[field]} must be greater than 0.`);
        }
      });
    ...
    function submitMetalIssue() {
      ...
      onSubmit({
        metalIssue: {
          ...
          source: metalSource,
          sourceLabel: getMetalSourceLabel(metalSource),
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          showFineGold
            ? h(ActualWeightInput, {
                label: isMixedSource ? "Actual Fine Gold Weight (Top-up)" : "Actual Fine Gold Weight",
                disabled: !canEditStage,
          ...
          showAlloy
            ? h(ActualWeightInput, {
                label: isMixedSource ? "Actual Alloy Weight (Top-up)" : "Actual Alloy Weight",
                disabled: !canEditStage,
  ...
  function OrderCompletedPanel({
    ...
    const suggestedFineGoldValue = hasSubmittedWeightValue(order.suggestedFineGoldRequired)
      ...
      : metalIssue.suggestedFineGoldRequired;
    ...
    const summarySections = [
      {
        ...
        items: [
          ...
          { label: "Required Fine Gold", value: formatSummaryWeight(metalIssue.referenceFineGold) },
          { label: "Required Alloy", value: formatSummaryWeight(metalIssue.referenceAlloy) },
          { label: "Actual Fine Gold Weight", value: formatSummaryWeight(order.actualFineGoldWeight) },
          { label: "Actual Alloy Weight", value: formatSummaryWeight(order.actualAlloyWeight) },
          ...
          { label: "Suggested Fine Gold Required", value: formatSummaryWeight(suggestedFineGoldValue) },
          ...
          { label: "Total Issued Weight", value: formatSummaryWeight(order.totalIssuedWeight) },
          {
            ...
            value: metalIssue.sourceLabel || (metalIssue.source ? getMetalSourceLabel(metalIssue.source) : "Not available")
          },
  ...
  function MixedMetalReferencePanel({ mixedReference, suggestedAlloyRequired, suggestedFineGoldRequired }) {
    return h(
      "div",
      {
        className:
      ...
      h(
        ...
        h(ReferenceValue, {
          label: "Target Pure Gold Required",
          value: formatReferenceWeight(mixedReference?.targetPureGoldRequired)
        }),
        h(ReferenceValue, {
          label: "Pure Gold In Recycled",
          value: formatReferenceWeight(mixedReference?.pureGoldAlreadyPresent)
        }),
        h(ReferenceValue, {
          label: "Suggested Fine Gold Required",
