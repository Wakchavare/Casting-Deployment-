...
function getActiveModuleLabel() {
  const activePanel = Array.from(modulePanels).find((panel) => !panel.hidden);
...
function submitMetalReceivingForm(form) {
  ...
  try {
    Inventory.submitMetalReceiving(
      {
    ...
    renderMetalReceiving();
    renderInventory();
    setMetalReceivingStatus("Receiving entry submitted and locked.");
  } catch (error) {
...
function deactivateSelectedUser() {
  ...
  const result = RBAC.deactivateUser(selectedUserId);
  if (!result) return;
...
function saveEditedRow(entryId, row) {
  if (!canUseModule("waxEntries", "edit")) {
    showAccessMessage(permissionDeniedMessage);
    return;
  }
